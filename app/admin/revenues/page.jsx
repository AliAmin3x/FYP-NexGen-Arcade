...
const Revenues = () => {
    ...
    useEffect(() => {
        const fetchGames = async () => {
            try {
                const q = query(collection(db, 'games'), where('status', 'in', ['Approved', null]));
            ...
            <div className='flex flex-1'>
                ...
                <div className="container mx-auto text-white py-8">
                    ...
                    {games.length === 0 && !error && (
                        <p className="text-center text-sm text-gray-300">No approved games found</p>
                    )}
                            ...
                            <tbody>
                                {games.map(game => (
                                    <tr key={game.id}>
                                        ...
                                        <td className="text-center py-2 px-4 border-b border-gray-700">${game.price.toFixed(2)}</td>
                                        <td className="text-center py-2 px-4 border-b border-gray-700">${(game.price * 0.20).toFixed(2)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
