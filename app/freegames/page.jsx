...
const FreeGames = () => {
  ...
  useEffect(() => {
    fetch("/api/games?type=free&status=Approved")
      ...
      .catch(() => toast.error("Error fetching free games"));
  }, []);
  ...
  return (
    <div className="bg-[#181818] flex flex-col min-h-screen">
      ...
      <div className="container mx-auto text-white text-center py-8">
        <motion.h2 className="text-4xl font-semibold mb-8" {...cardAnimation}>Free Games</motion.h2>
          ...
          {paginatedGames.length > 0 ? (
            paginatedGames.map((game) => (
              ...
              >
                ...
                >
                  {favorites[game.id] ? (
                    <AiFillHeart size={24} className="cursor-pointer text-purple-400" />
                  ) : (
                    <AiOutlineHeart size={24} className="cursor-pointer text-purple-400" />
                  )}
                </motion.div>
              </motion.div>
            ))
          ) : (
            <p className="col-span-4 text-gray-400">No free games available at the moment.</p>
          )}
        </motion.div>
